PROGRAM MIX_random
    implicit none
    INTEGER :: I,nsubj,ndatasets,r,k,seed(1),nonpos,mysize,j,ns,numre,numre2,mytotal
    REAL(KIND=8) :: random_normal
    REAL(KIND=8),ALLOCATABLE:: chols(:,:),means(:,:),temp(:),myrand(:,:),temp2(:),cur_val(:)!,current(:,:)
    CHARACTER(LEN=80) :: fileebmeans,fileout

    OPEN(1, FILE='mix_random.def')
    READ(1,*)FILEebmeans
    READ(1,*)FILEout
    READ(1,*) Nsubj, r, ns, ndatasets, seed
    close(1)
    ...
    mysize=1
!    allocate(current(nsubj,r))
    allocate(myrand(nsubj,numre))
    !call random_seed(size=mysize,put=seed)
