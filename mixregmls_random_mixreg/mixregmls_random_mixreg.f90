...
!
    ...
    ! and the random scale parameters
        ...
        REAL(KIND=8)::TP2(((S+1)*S)/2),TP3(S)        ! temporary array
        REAL(KIND=8)::DET,ERRV,RTEMP,SMALL,fitted(nobs),tau0
        INTEGER:: ier, i, k, j, c

        RTEMP = 1.0D0
        tau0 = tau(1)
        SMALL  = TINY(RTEMP)
        CALL MPYM(X,BETA,fitted,NOBS,P,0,0,1)
        c = 1
        write(*,*) (chol(k),k=1,r*(r+1)/2)
        do i = 1,nc2
            do j=1,idni(i,2)
                do k=1,r
...
! estimation subroutine 
    SUBROUTINE mixregmlsEST(IDNI,Y,X,U,W,BLAB,ALAB,TLAB,NC2,P,R,S,CONV,NQ,AQUAD,MAXIT,NCENT,RIDGEIN, &
        ...
        INTEGER :: S,I,J,L,LL,l2,k,PP,RR,SS, &
        ...
        ndim2 = (ndim+1)*ndim/2
        IUN    = 16
        OPEN(UNIT=IUN,FILE="MIXREGmLS2.OUT")
        IUNS    = 17
...
!               spar(1) = .1
        ...
        ! START WITH NEWTON RAPHSON (MIXREG for starting values)
            ...
            !if(ns == 0) write(IUNS,'(18F10.3)') i*1.0, q*1.0, psum, rtemp2,pra,h(i),(derq(k), k=1,npar)
                ...
                ll = 0
                do k=1,npar
                    write(IUNS,'(25f10.2)') (der2b(ll+l), l=1,k)
                    ll = ll + k
                end do
                ...
                write(IUNS,*) (der(k), k=1,npar)
                write(IUNS,*)"Beta"
                write(IUNS,*) (beta(k), k=1,p)
                write(IUNS,*)"Chol"
                write(IUNS,*) (mychol(k),k=1,rr)
                write(IUNS,*)"Tau"
                write(IUNS,*) (tau(k),k=1,s)
                if(ns > 0) then
                    write(IUNS,*)"Spar"
                    write(IUNS,*) (spar(k),k=1,ns)
                end if
...
!     END DO
     ...
     ! write out the deviance, estimates, and standard errors
        WRITE(2,'(F15.6,2I8)') -2*LOGL,ITER-1,MAXIT
         ...
         WRITE(2,'(5F15.8)')(mychol(L),L=1,RR)
...
!    close(29)
        ...
        deallocate(sstar,sstar2,sstar2t,work,adjvar,asstar2,asstar2t,sigma)
    END SUBROUTINE mixregmlsEST


END PROGRAM mixregmls_subject
