...
MODULE mixor_globals
    ...
    INTEGER :: NVAR,NQ,AQUAD,ID2IND,YIND,P,R,S,RR,rnint,pv,rv,sv,nv, &
               ...
               myseed,stage2,multi2nd,pfixed,ptheta,pomega,pto,readcats,nvar3,maxj2,nvar2,&
               numloc,sepfile,nvarsep,id2indsep
    INTEGER,ALLOCATABLE :: XIND(:),UIND(:),WIND(:),Yasint(:),ids(:),ni(:), cluster_nob(:,:), catcount(:),varind(:)
    ...
    CHARACTER(LEN=16),ALLOCATABLE :: BLABel(:),tLABel(:),clabel(:),varlabel(:),var2label(:)
...
#else
    CALL SYSTEM("cp mixors.OUT " // trim(fileprefix)//".out")
    CALL SYSTEM("rm mixors_all.est")
    CALL SYSTEM("rm mixors_all.var")
    CALL SYSTEM("rm mixors_details.its")
#endif
end program mixors
...
subroutine readDef
    ...
    ns = r+1
    ngamma = maxj-1
    NPAR = P+RR+ngamma+S+NS ! number of parameters
    CLOSE(1)
...
subroutine writeDef
    ...
    write(1,*) NVAR, maxj, Pold, Rold, Sold, rnint, pv, rv, sv, CONV, NQ, AQUAD, MAXIT, yMISS, ridgein, nfn
    write(1,*) chol, iadd
...
200  FORMAT(1x,A16,4F12.4)
    ...
    WRITE(17,258) YLabel
    258 FORMAT(//,1x,'Categories of the response variable ',A16, &
               /,1x,'--------------------------------------------',/)
    WRITE(17,457)
...
SUBROUTINE MIXorEST()
    ...
    REAL(KIND=8) :: RIDGE,LOGLP,LOGL, &
                    ...
                    COREC(npar), temp(npar,npar), myider2(npar,npar),&
                    myqderp2(nobs,npar,npar),myider(npar),myqder2(npar,npar),nopoints(1,1),noweights(1)
    ...
    IUN    = 16
    IUNS    = 17
    OPEN(UNIT=IUNS,FILE="MIXorS_details.ITS")
    open(19,file="MixorS_all.var")
    open(13,file="MixorS_all.est")
    if(aquad .ne. 0) open(29,file=trim(fileprefix)//"_ebvar.dat")
    qtotalR0 = nq ** R
    qtotalR1 = nq ** (R+1)
    allocate(pointsR0(qtotalR0,r))
    allocate(pointsR1(qtotalR1,r+1))
    allocate(weightsR0(qtotalR0))
    allocate(weightsR1(qtotalR1))
    call getquad(r, nq, qtotalR0, pointsR0, weightsR0)
    call getquad(r+1, nq, qtotalR1, pointsR1, weightsR1)
    nopoints(1,1) = 0
    noweights(1) = 1
...
! cycles = 4: use NS = R+1
    ...
    CYCLELOOP:do cycles=1,ncycle
        OPEN(UNIT=IUN,FILE="MIXorS.OUT",access="append")
...
!            WRITE(*,*) "------------------------------------------------------"
        ...
        NPAR_cycle = P+RR_cycle+ngamma+S_cycle+NS_cycle ! number of parameters
!
! start iterations
!
    ...
    ! START WITH NEWTON RAPHSON (MIXREG for starting values)
        ...
        IFINLOOP:DO WHILE (ifin < 3)
            ...
            if(iter >= 10 .and. (maxval(abs(myder(1:npar_cycle))) < 2.5)) then! .or. maxval(abs(corec(1:npar_cycle)))<conv*10)) then
                ...
                if (cycles <= 3 .and. iter >= 5+cycles*5 .and. maxval(abs(myder(1:npar_cycle))) < 10) then
                    ridge = 0
                else
            ...
            ILOOP:DO I=1,NClust  ! go over level-2 clusters
                ...
                QLOOP:DO Q=1, mytotalq  ! go over quadrature points
                    ...
                    jloop: do j=1,ni(i)
                        ...
                        t=0
                        do k=1, min(R,RR_cycle) !to account for the case when RR_cycle=0
                            do m=1, k
                                t = t + 1
                        ...
                        if(ns_cycle > 0) then
                            if(ns_cycle > 1) then
                                ...
                                dlambda_upper1(npar_cycle,1) = -mypoints(q,R+1)*lambda_upper
                                dlambda_lower1(npar_cycle,1) = -mypoints(q,R+1)*lambda_lower
                            end if
                        end if
                                ...
                                do k=1,s_cycle+ns_cycle
                                    if(h <= t+k) then
                                        if(k<=s_cycle) then
                                            ...
                                            kk = k - s_cycle
                                            if(kk==ns_cycle) then
                                                myqderp2(j,npar_cycle,h) = myqderp2(j,npar_cycle,h) - mypoints(q,R+1)* &
                                                        (pdf_upper*dlambda_upper1(h,1)-pdf_lower*dlambda_lower1(h,1))
                                            else
...
!            write(*,'(25f9.3)') (corec(k), k=1,npar_cycle)
            ...
            if (s_cycle > 0) TAU   = TAU      + COREC(P+RR_cycle+ngamma+1:P+RR_cycle+ngamma+S_cycle)
            if(RR_cycle > 0 .and. iter > 14) then
            ...
            if(ns_cycle > 0 .and. (iter > 10 .or. (iter > 5 .and. cycles >= 5))) then
                do k=1,ns_cycle
...
!                    end if
        ...
        if(cycles .ne. 4) then
            if(rr_cycle > 0 .and. chol .ne. 2) then
                cholamt = rr_cycle
    ...
    write(iuns,'(20f6.2)') (mycholspar(j),j=1,cholamt)
                if(chol .ne. 2) then
                    call getSStar(mycholspar,r+(1-chol)*min(1,ns_cycle),cholamt,sstar)
    ...
    do i=1,npar
        write(iuns,'(20F15.8)') (temp(i,j), j=1,npar)
    end do
                    work(1:npar,1:npar) = matmul(asstar2(1:npar,1:npar), temp(1:npar,1:npar))
    ...
    !            sigma = matmul(sstar,mychol)
    !            asstar2 = 0
    !            do j=1,npar_cycle
    !                asstar2(j,j) = 1
    !            end do
    !            asstar2(p+ngamma+1:p+ngamma+rr_cycle,p+ngamma+1:p+ngamma+rr_cycle) = 2*sstar
    !           asstar2t = transpose(asstar2)
    !            work(1:npar_cycle,1:npar_cycle) = matmul(asstar2(1:npar_cycle,1:npar_cycle), temp(1:npar_cycle,1:npar_cycle))
    !            adjVar(1:npar_cycle,1:npar_cycle) = matmul(work(1:npar_cycle,1:npar_cycle),asstar2t(1:npar_cycle,1:npar_cycle))
    !            if(r > 1) then
    !                write(iun,*) "Matrix to transform cholesky back"
    !                write(iun,'(20f6.2)') ((sstar(i,j),i=1,rr),j=1,rr)
    !                write(iun,*) "Original var/covar values"
    !                write(iun,'(20f8.2)') ((temp(p+ngamma+i,p+ngamma+j),i=1,rr),j=1,rr)
    !               write(iun,*) "Adjusted var/covar values"
    !                write(iun,'(20f10.2)') ((adjvar(p+ngamma+i,p+ngamma+j),i=1,rr),j=1,rr)
    !            end if
    !        else
    !            sigma = mychol
    !            adjvar(1:npar_cycle,1:npar_cycle) = temp(1:npar_cycle,1:npar_cycle)
    !        end if
    !        do k=1,npar_cycle
    !            se(k) = dsqrt(dabs(adjvar(k,k)))
    !        end do
            ...
            write(19,*)
             ...
             WRITE(13,'(35F15.8)')(sigma(myorder(k)),k=1,RR_cycle)
             WRITE(13,'(35F15.8)')(TAU(k),k=1,S_cycle)
             write(13,'(35F15.8)')(sigma(myorder(rr+k)),k=1,ns_cycle)
...
!             (se(k),k=p+rr+1,p+rr+s),(se(myorder2(k)),k=rr+1,rr+ns)
        ...
        805 FORMAT(A10,I0,I0,4X,4(4x,F12.5))
            ...
            IF (ngamma .GT. 0) THEN
                ...
                DO k = 1,ngamma
                    ...
                    PVAL = 2.0D0 *(1.0D0 - PHIFN(DABS(ZVAL),0))
                    WRITE(IUN,'(I2,14X,4(4x,F12.5))') k,mygamma(k),SE(k2),ZVAL,PVAL
                END DO
            ENDIF
             ...
             IF (cycles>=4) THEN
                ...
                L2=P+RR_cycle+S_cycle+ns_cycle+ngamma
                if(spar(ns_cycle) < 0) spar(ns_cycle)=abs(spar(ns_cycle))
    ...
    END DO CYCLELOOP
     ...
     close(19)
    close(13)
    open(19,file="MixorS.var")
    open(13,file="MixorS.est")
            ...
            do k=1,npar_cycle
             ...
             WRITE(13,'(a24,F15.8)')("VarCov",sigma(myorder(k)),k=1,RR_cycle)
             WRITE(13,'(a24,F15.8)')(tlabel(k),TAU(k),k=1,S_cycle)
             write(13,'(a24,F15.8)')("VarCov",sigma(myorder(rr+k)),k=1,ns_cycle)
...
!     Scale linear predictor
    ...
    if (ns_cycle > 0) then
        wt = wt + mypoints(q, R+1)*spar(ns_cycle)
        do k=1, ns_cycle-1
