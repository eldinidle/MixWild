...
PROGRAM MIXREGmLS
implicit none
INTEGER :: I,NOBS,NVAR,NQ,AQUAD,ID2IND,YIND,P,R,S,PP,RR,SS, &
           MISS,MAXK,NC2,MAXIT,NCENT,PNINT,RNINT,SNINT,POLD,ROLD,SOLD,&
           myio,j,iun,npar,counter,rr1,pv,rv,sv,nv,ll,ko,h,kv,ncov,num0,discard0
INTEGER,ALLOCATABLE :: XIND(:),UIND(:),WIND(:),IDNI(:,:),varIND(:),ids(:)
REAL(KIND=8) :: RIDGEIN,MEANY,STDY,CONV,YMISS,MINY,MAXY,SDLV,RCORR,RTEMP,&
                temp,pval,zval,phifn,logl,rsames
REAL(KIND=8),ALLOCATABLE:: TEMPR(:),Y(:),X(:,:),U(:,:),W(:,:), &
                           MEANX(:),STDX(:),MEANU(:),STDU(:),MEANW(:),STDW(:), &
                           MINX(:),MAXX(:),MINU(:),MAXU(:),MINW(:),MAXW(:), &
                           BETA(:),TAU(:),SPAR(:), &
                           IDMV(:,:),mychol(:),se(:),temp3(:),v(:),&
                           thetas(:,:),thetavs(:,:),var(:,:),varavg(:,:)
CHARACTER(LEN=16) :: YLABEL, templabel
CHARACTER(LEN=16),ALLOCATABLE :: BLAB(:),ALAB(:),TLAB(:)
character(len=12),allocatable::varlabel(:)
CHARACTER(LEN=4) :: HEAD(36)
CHARACTER(LEN=80) :: FILEDAT, FILEOUT, FILEDEF,fileprefix
CHARACTER(LEN=160) :: FILEOUT2
LOGICAL FP_EQUAL, longdef


    OPEN(1, FILE='mixregmls_var.def')
    READ(1,9) HEAD
 9  FORMAT(18A4)
    READ(1,5)FILEDAT
    READ(1,5)FILEprefix
 ...
 fileout = trim(fileprefix)//".out"
    ...
    sv = 0
    longdef = .false.
    READ(1,*) NVAR, P, R, S, PNINT, RNINT, SNINT, CONV, NQ!, temp, MAXIT, MISS, NCENT, RIDGEIN
...
CALL READAT(FILEDAT,NC2,NOBS,MAXK,NVAR,R,P,S,nv,Y,X,U,W,var,varavg, &
            IDNI,ID2IND,YIND,XIND,UIND,WIND,varind,MISS,YMISS, &
            pold,rold,sold,PNINT,RNINT,SNINT,ids,discard0)
...
call checkeij(y,idni,nc2,nobs,p,x,num0)

    OPEN(1,FILE="mixreg.def")
    WRITE(1,9) HEAD
    WRITE(1,'("mixreg_temp.dat")')
    write(1,'("temp_.out")')
    write(1,'("temp_.def")')
    write(1,'("mixreg.res")')
...
!       call chams(mychol,v,r,1,0)
       ...
       open(unit=1,file="mixreg.res")
       rr1 = (r+1)*(r+2)/2
    ALLOCATE (THETAs(NC2,r+1))
    ALLOCATE (thetavs(NC2,rr1))
       do i=1,nc2
    ...
    IUN    = 16
    OPEN(UNIT=IUN,FILE="mixregmls53.OUT")
               RSAMES = 100.0D0 * (DBLE(num0) / DBLE(nc2))
                  WRITE(IUN,508)num0,RSAMES
...
v = mychol
       ...
       CALL mixregmlsEST(IDNI,Y,X,U,W,BLAB,ALAB,TLAB,NC2,P,R,S,CONV,NQ,AQUAD,MAXIT,NCENT,RIDGEIN, &
                        BETA,TAU,SPAR,mychol,thetas,thetavs,snint,ncov)

       FILEOUT2 = "COPY mixregmls51.OUT+mixregmls53.OUT+mixregmls52.OUT " // FILEOUT
       CALL SYSTEM(FILEOUT2)
    CALL SYSTEM("DEL mixREGmLS51.OUT mixregmls53.out mixregmls52.out")
...
SUBROUTINE READAT(FILEDAT,NC2,NOBS,MAXK,NVAR,R,P,S,nv,Y,X,U,W,var,varavg, &
    IDNI,ID2IND,YIND,XIND,UIND,WIND,varind,miss,YMISS,pold,rold,sold,pnint,rnint,snint,ids,discard0)
    implicit none
    ...
    INTEGER :: myPASS,I,K,ICOUNT,myindex,IDTEMP,IDOLD,hasmiss,nvartotal,discardi,icount1
...
INTEGER:: IUN,I
     ...
     IUN    = 16
     OPEN(UNIT=IUN,FILE="mixregmls51.OUT")
     
     WRITE(IUN,'("mixregmls: Mixed-effects (multiple) Location Scale Model")')
     write (IUN,*)
     WRITE(IUN,'("-----------------------------")')
     WRITE(IUN,'("mixregmls.DEF specifications")')
     WRITE(IUN,'("-----------------------------")')
...
INTEGER,INTENT(IN)::NC2,P,R,NQ,AQUAD,MAXIT,NCENT,S,snint,ncov                        
INTEGER,INTENT(IN),DIMENSION(:,:)::IDNI                                     ! ids and nobs per cluster
REAL(KIND=8),INTENT(IN),DIMENSION(:)::Y                      ! input matrix Y, and random location mean and posterior variance
REAL(KIND=8),INTENT(INOUT),DIMENSION(:)::BETA,TAU,SPAR,mychol         ! parameters (mean, bs var, ws var, random scale)
...
INTEGER :: I,J,L,LL,l2,k,PP,RR,SS, &
           NPAR,IUN,CYCLES,NCYCLE,RP2,RPS2,NQ1,NS,NS2,IFIN,ITER,NN,NPAR2, &
           Q,NOB,IER,RIDGEIT,IUNS,NQwR0,NQwR1,ndim,ndim2,myqdim,totalqR0,totalqR1,mytotalq,&
           pabove, pbelow, asCount, sCount
REAL(KIND=8) :: RIDGE,LOGLP,PSUM,DET,LOGL,PI,XB,WT, &
                WSVAR,ERRIJ,LPROB,PRA,LOGDIFF,MAXCORR,ONE,SCAL,&
                ORIDGE,PVAL,ZVAL, &
                RTEMP,RTEMP2,SMALL,BIG,LOGBIG,MAXDER,PHIFN,sdev,phiRatio,uchth,&
                tauhat, tauhatlow, tauhatup, myz
REAL(KIND=8),ALLOCATABLE:: DER2A(:),DER2B(:),DER(:),LIK(:,:),H(:), &
                           ...
                           mypoints0(:,:),pointsR0(:,:),weightsR0(:),pointsR1(:,:),weightsR1(:),cholTheta(:),uth(:),uthfull(:),&
                           sstar(:), sstar2(:), work(:), sstar2t(:), sigma(:), asstar2(:), adjVar(:), asstar2t(:), prevcorec(:)
CHARACTER(LEN=16),INTENT(IN),DIMENSION(:):: BLAB(:),ALAB(:),TLAB(:)
    ...
    ndim2 = (ndim+1)*ndim/2
    IUN    = 16
    OPEN(UNIT=IUN,FILE="mixregmls52.OUT")
    IUNS    = 17
    OPEN(UNIT=IUNS,FILE="mixregmls_.ITS")
    open(unit=18, file="Mixregmls_.DEV")
    NQwR1 = NQ**(R+1)
    nqwR0 = nq**R
...
! cycles = 4: use NS = R+1
    ...
    CYCLELOOP:do cycles=2,ncycle
        if(cycles==2 .and. s==1 .and. snint == 0) cycle
    ...
    ! START WITH NEWTON RAPHSON (MIXREG for starting values)
        ...
        !        if(aquad .ne. 0 .and. (iter >= 8 .or. cycles .eq. 4)) then
               ...
               QLOOP:DO Q=1, mytotalq  ! go over quadrature points
                 ...
                 JLOOP:DO J=1,IDNI(I,2)  ! loop over level-1 observations
                        ...
                        call mpym(mychol, mypoints(q,1:R), cholTheta, R, R, 3, 0, 1)
        ...
        !if(ns == 0) write(IUNS,'(18F10.3)') i*1.0, q*1.0, psum, rtemp2,pra,h(i),(derq(k), k=1,npar)
             ...
             IF (LOGDIFF/LOGL > .05 .AND. ITER < MAXIT) THEN
                RIDGEIT = 0
                RIDGE = RIDGE + .05D0
             ...
             IF (LOGDIFF/LOGLP <= .000001 .AND. RIDGEIT < 10) THEN
                RIDGEIT = RIDGEIT+1
             ELSE IF (LOGDIFF/LOGL <= .000001 .AND. RIDGEIT >= 10 .and. ifin==1) then
                    ridge = ridge - .05D0
                    if(maxder < 1) then
                        ridge = 0
                    else
...
99           if(ifin == 1) then
                ...
                if(ns > 0) then
                 ...
                 WRITE(18,'(2f15.7)') MAXCORR,MAXDER
            ...
            !     END IF
                 ...
                 if(iter > 20) mychol = mychol + COREC(P+1:P+RR)*.5
                    ...
                    do k=1, r
                        do L=1, k
                            if(k==L .and. mychol(LL) < 0) mychol(LL) = -mychol(LL)
                            LL = LL + 1
                        end do
                    end do
            end if
         ...
         END DO IFINLOOP
                ...
                call mpym(sstar,mychol,sigma,RR,RR,0,0,1)
            ...
            call mpym(asstar2,der2b,work,npar,npar,0,1,npar)
            call mpym(work,asstar2t,adjVar,npar,npar,0,0,npar)

            DO L=1,npar
                ll = npar*(l-1)+l
               SE(L)=DSQRT(DABS(adjVar(LL)))
            END DO
    ...
    57 FORMAT(/,'Variable',12x,'    Estimate',4X,'AsymStdError',4x, &
        ...
        counter = 1
         do i=1,r
             do j=1,i
                ZVAL = sigma(counter)/SE(counter+p)
                PVAL = 2.0D0 *(1.0D0 - PHIFN(DABS(ZVAL),0))
                if(i==j) then
                    WRITE(IUN,804)alab(i),sigma(counter),SE(counter+p),ZVAL,PVAL
                else
                    WRITE(IUN,805)'Covariance',j,i,sigma(counter),SE(counter+p),ZVAL,PVAL
                end if
                counter = counter+1
            end do
         END DO
        ...
        write(iun,*)
         ...
         if(cycles == 4) then
            ...
            do k=1,ns-1
                ...
                ZVAL = SPAR(k)/SE(L2)
                PVAL = 2.0D0 *(1.0D0 - PHIFN(DABS(ZVAL),0))
                WRITE(IUN,804)ALAB(k),spar(k),SE(L2),ZVAL,PVAL
            end do
        end if
    ...
    !     END DO
         ...
         IF (MAXDER > CONV .AND. ITER >= MAXIT) THEN
            ...
            END DO
         END IF
    !    write(ebname,"(a2,i1,a4)") 'eb',cycles,'.dat'
    !    open(23,file=trim(fileprefix)//"_"//ebname)
    !    k=myqdim*(myqdim+1)/2
    ...
    !	close(23)
         ...
         ! write out the deviance, estimates, and standard errors
            WRITE(2,'(F15.6,2I8)') -2*LOGL,ITER-1,MAXIT
             ...
             WRITE(2,'(5F15.8)')(sigma(L),L=1,RR)
             WRITE(2,'(5F15.8)')(TAU(L),L=1,S)
             write(2,'(5F15.8)')(SPAR(L),L=1,ns)
             WRITE(2,'(5F15.8)')(SE(L),L=1,NPAR)
    close(2)
    OPEN(2, FILE='mixregmls_.EST',position='append')
    close(iun)
        OPEN(UNIT=IUN,FILE="mixregmls52.OUT",position='append')
     END DO CYCLELOOP
...
!                      COREC,DZ2,SE,sbder2,sader2,stder2,sbader2,theta,mytheta,psd,thetaa,psda,sbatder2,sder2, &
!                      lik,h,tbder2,tader2,tbader2,tder2,theta2,thetav,thetaa2,thetaav,work2,work3)
END SUBROUTINE mixregmlsEST

subroutine checkeij(y,idni,nc2,nobs,p,x,num0)
    implicit none
    INTEGER,INTENT(IN)::NC2,P,nobs                        
    INTEGER,INTENT(IN),DIMENSION(:,:)::IDNI                                     ! ids and nobs per cluster
    REAL(KIND=8),INTENT(IN),DIMENSION(:)::Y                      ! input matrix Y, and random location mean and posterior variance
    REAL(KIND=8),INTENT(IN)::X(nobs,p)         ! parameters
    integer,intent(out)::num0

    REAL(KIND=8),ALLOCATABLE:: eij(:),betahat(:),xp(:,:),xprimex(:,:),xpxi(:,:),xpy(:),yhat(:),ei(:),meanei(:),varei(:),ei2(:),&
                                yi(:),meanyi(:),varyi(:),yi2(:)
    integer:: i,j,counter,ni

   allocate(yHat(nobs))
    allocate(xp(p,nobs))
    allocate(xprimex(p,p))
    allocate(xpxi(p,p))
    allocate(xpy(p))
    allocate(eij(nobs))
    allocate(betahat(p))
    xp = transpose(x(1:nobs,:))
    xprimex = matmul(xp,x(1:nobs,:))
   call inverse(xprimex,xpxi,p)
   xPY = matmul(xP,y(1:nobs))
   betahat = matmul(xpxi,xpy)
   yHat = matmul(x(1:nobs,:), betahat)
   eij = y(1:nobs) - yHat
   allocate(ei(nobs))
   allocate(yi(nobs))
   allocate(meanei(nc2))
   ...
   do i=1,nc2
       ...
       do j=1,ni
        ...
        meanei(i) = sum(ei(1:ni))/ni
        ei2 = ei - meanei(i)
        varei(i) = dot_product(ei2(1:ni),ei2(1:ni))/(ni-1)
        meanyi(i) = sum(yi(1:ni))/ni
        yi2 = yi - meanyi(i)
        varyi(i) = dot_product(yi2(1:ni),yi2(1:ni))/(ni-1)
        write(23,'(i12,i4,2f10.3,2f10.3,f10.3)') idni(i,1),ni,meanei(i),varei(i),meanyi(i),varyi(i),log(varyi(i)+1) !(ei(j),j=1,ni)
        if(fp_equal(varyi(i),0.0d0)) then
!            write(23,*) "subject with 0 outcome variance!"
            num0 = num0 + 1
        end if
    end do    
   close(23)
   end subroutine checkeij
END PROGRAM mixregmls
...
END SUBROUTINE CHSKY
    ...
    subroutine inverse(a,c,n)
        ...
        real(kind=8)::L(n,n),U(n,n),b(n),d(n),x(n),work(n,n)
        real(kind=8)::coeff
        integer::i,j,k
        ...
        do k=1, n-1
            do i=k+1,n
                  coeff=work(i,k)/work(k,k)
                  L(i,k) = coeff
                  do j=k+1,n
                     work(i,j) = work(i,j)-coeff*work(k,j)
                  end do
            end do
        end do
