...
! read in the labels
    ...
    IF (Rv .GE. 1) THEN
     ...
     if(no2nd .ne. 0) then
         ...
         read(1,*) pfixed,ptheta,pomega,pto,maxj
         if(nors .ne. 0) then
            pomega = -1
            pto = -1
            nors = 1
...
!    WRITE(1,5)FILEDEF
    ...
    IF (Rv .GE. 1) THEN
     ...
     END IF
     if(no2nd .ne. 1) then
         write(1,*) pfixed,ptheta,pomega,pto,maxj
        write(1,*) var2ind(1)
...
! get descriptive stats on all of the variables
 ...
 805 FORMAT(A10,I0,I0,4X,4(4x,F12.5))
    ...
    if(no2nd .ne. 1) then
        ...
        write(1,*) nvar2+1,nreps,0,pfixed,ptheta,pomega,pto,maxj,cutoff
        write(1,*) 1, 2
            k=2
            if(pfixed .ge. 1) then
...
! estimation subroutine 
    SUBROUTINE mixregmlsEST(IDNI,Y,X,U,W,BLAB,ALAB,TLAB,NC2,P,R,S,CONV,NQ,AQUAD,MAXIT,NCENT,RIDGEIN, &
        ...
        INTEGER :: S,I,J,LL,l2,k,kk,PP,RR,SS, &
...
!     END DO
     ...
     ! write out the deviance, estimates, and standard errors
        WRITE(2,'(F15.6,2I8)') -2*LOGL,ITER-1,MAXIT
         WRITE(2,'(5F15.8)')(BETA(k),k=1,P)
         WRITE(2,'(5F15.8)')(mychol(k),k=1,RR)
         WRITE(2,'(5F15.8)')(TAU(k),k=1,S)
         write(2,'(5F15.8)')(SPAR(k),k=1,ns)
         WRITE(2,'(5F15.8)')(SE(k),k=1,NPAR)
