...
PROGRAM MIXREGLS_subject
    ...
    OPEN(1, FILE='mixregls_random_mixor.def')
    READ(1,9) HEAD
    9 FORMAT(18A4)
    READ(1,*)FILEDAT
    READ(1,*)FILEprefix
...
! NOTE THAT Y, X, U, W, IDNI ARE ALLOCATED IN READAT
    CALL READAT(FILEDAT,NC2,NOBS,MAXK,NVAR,R,P,S,nv,nvar2,Y,X,U,W,&
            var,varavg,tempsums,IDNI,ID2IND,YIND,XIND,UIND,WIND,&
            varind,var2ind,MISS,YMISS,pold,rold,sold,pnint,rnint,&
            snint,discard0,num0,fileprefix)
 ...
 ! get starting values
    ...
    if(no2nd .ne. 1) then
        ...
        DO I=1,NC2  ! go over level-2 clusters
            write(32,'(i16,100f15.6)') idni(i,1),(tempsums(i,k),k=1,nvar2)
        end do
        close(32)
...
!        RR = ((R+1)*R)/2
        ...
        IUNS    = 17
        OPEN(UNIT=IUNS,FILE="mixREGLS5_detailed.ITS")
